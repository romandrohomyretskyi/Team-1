...
class Road:
    ...
    def draw(self,size,step):
        ...
        while i<lineLength-1:
            ...
            arrayPoint.append(Tablet(position=positionStart,size=size,entity=Entity(model="sphere",color=Vec4(1,1,1,1))) if randint(1,100)>1 else SuperTablet(position=positionStart,size=size,entity=Entity(model="sphere",color=Vec4(1,1,0,1))))
